...
//
//    @Test
//    fun `getHexagonsFromCity returns Success when network data is fetched`() =
//        runBlocking {
//            val city = "Poznań"
//            val key = "get-hexagons-from-city-$city"
//            val remoteVersion = "1"
//            val mockData = GetHexagonsFromCityResponseDto(
//                bbox = listOf(1.0, 2.0, 3.0, 4.0),
//                hexagons = emptyList()
//            )
//
//            coEvery { versionApiClient.getCurrentVersion(key).body()?.version } returns remoteVersion
//            coEvery { cacheService.getCachedVersion(key) } returns "v1"
//
//            coEvery { hexagonApiClient.getHexagonsFromCity(city).body() } returns mockData
//
...
//
//    @Test
//    fun `getHexagonsFromCity returns Fallback when network fails but cache exists`() =
//        runBlocking {
//            val city = "Poznań"
//            val key = "get-hexagons-from-city-$city"
//            val cachedData = GetHexagonsFromCityResponseDto(bbox = listOf(1.0, 2.0), hexagons = emptyList())
//
//            coEvery { versionApiClient.getCurrentVersion(key) } throws Exception("No internet")
//
//            coEvery { cacheService.getCachedData<GetHexagonsFromCityResponseDto>(key, any()) } returns cachedData
//
//            val result = repository.getHexagonsFromCity(city)
//
...
//
//    @Test
//    fun `postLocationBatch returns true and emits updates on success`() =
//        runBlocking {
//            val mockToken = "old_token"
//            val newToken = "new_token"
//            val locations = PostLocationBatchRequestDto(emptyList())
//            val updates = listOf(HexagonProgress("hex_1", 0.5))
//            val mockResponse = PostLocationBatchResponseDto(updatedHexagons = updates, token = newToken)
//
...
//
//    @Test
//    fun `postLocationBatch throws InvalidTokenException when token is null`() =
//        runBlocking {
//            coEvery { tokenService.getToken() } returns null
//
//            try {
//                repository.postLocationBatch(PostLocationBatchRequestDto(emptyList()))
...
//
//    @Test
//    fun `getHexagonProgresses clears token and throws InvalidTokenException on 401 error`() =
//        runBlocking {
//            val city = "Poznan"
...
//
//    @Test
//    fun `getHexagonProgresses returns list when token is valid`() =
//        runBlocking {
//            val city = "Poznań"
//            val mockToken = "valid_token"
//            val expectedProgress = listOf(HexagonProgress("hex_1", 1.0))
//
//            coEvery { tokenService.getToken() } returns mockToken
//            coEvery { hexagonApiClient.getHexagonProgresses("Bearer $mockToken", city).body() } returns expectedProgress
//
